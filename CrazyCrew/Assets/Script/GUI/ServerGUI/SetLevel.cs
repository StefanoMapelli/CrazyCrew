	...
	void OnMouseDown() {
		...
		if (level == 1) {
			if (PlayerPrefs.HasKey ("highmin1") && PlayerPrefs.HasKey ("highsec1") && PlayerPrefs.HasKey ("highmsec1")) 
				highscore.text = string.Format("Record: {0:D2}:{1:D2}:{2:D3}",PlayerPrefs.GetInt ("highmin1"),PlayerPrefs.GetInt ("highsec1"),PlayerPrefs.GetInt ("highmsec1"));
			else
				highscore.text = "No record";
		}
		else if (level == 2) {
			if (PlayerPrefs.HasKey ("highmin2") && PlayerPrefs.HasKey ("highsec2") && PlayerPrefs.HasKey ("highmsec2")) 
				highscore.text = string.Format("Record: {0:D2}:{1:D2}:{2:D3}",PlayerPrefs.GetInt ("highmin2"),PlayerPrefs.GetInt ("highsec2"),PlayerPrefs.GetInt ("highmsec2"));
			else {
